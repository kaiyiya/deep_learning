...
> *在此插入系统架构示意图（Web-Node-Python-Data 的交互流程与网络拓扑）*  
> ![系统架构图]( )
...
> *流程图插入位置：Node 触发 Python 构建 / 查询的时序图*  
> ![跨语言通信时序图]( )
...
1. **轻量级索引方案**  
   使用 NPZ + JSON 存储特征与元信息，不依赖数据库，适用于离线或受限环境。

2. **自动容错机制**  
   `ensureIndexReady` 在服务启动及检索前自动构建索引，提高系统稳定性。

3. **跨栈协同设计**  
   Node.js 与 Python 模块分工明确，通过标准接口通信，实现快速开发与深度模型利用。

4. **可视化与交互体验**  
   前端提供数据统计、图库浏览、检索结果统一界面，支持拖拽上传、实时状态反馈。
...
1. **示例查询一**  
   - 输入图像：某种红色花卉。  
   - 返回结果：前 5 名均为同类别花卉，背景色相近，相似度最高为 0.82。  
...
| 安装 Python 依赖 | `python -m pip install --trusted-host pypi.org --trusted-host files.pythonhosted.org -r python\requirements.txt` |
| 下载数据集 | `npm run download-data` |
| 构建索引 | `npm run build-index` |
| 开发模式 | `npm run dev` |
| 启动服务 | `npm start` |
| 清理上传缓存 | 自动删除（在 `/api/search` 结束后） |
...
*注：本报告中需要插入图像的位置已留空，请根据实际运行截图替换对应占位符。*
